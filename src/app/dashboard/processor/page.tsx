            ...
            <div className="min-h-screen min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="card p-8 text-center">
                        ...
                        <h2 className="text-xl font-bold text-slate-900 mb-2">Agri Officer Access Required</h2>
                        <Link href="/login?role=processor"><button className="btn btn-primary text-white mt-4 px-6 py-2 rounded-xl">Login as Agri Officer</button></Link>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="min-h-screen min-h-screen">
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 mb-1" style={{ fontFamily: "Outfit, sans-serif" }}>?? Agri Officer Dashboard</h1>
                        <p className="text-[#475569]">Welcome, <span className="text-blue-400">{user?.name}</span></p>
                    </div>
                    <div className="flex bg-[#ffffff] p-1 rounded-xl border border-slate-200">
                        <button onClick={() => setActiveTab('inspection')} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === 'inspection' ? 'bg-green-600 text-white shadow-lg' : 'text-[#475569] hover:text-white'}`}>
                            ?? Crop Inspection
                        </button>
                        <button onClick={() => setActiveTab('quality')} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === 'quality' ? 'bg-blue-600 text-white shadow-lg' : 'text-[#475569] hover:text-white'}`}>
                            ?? Quality Tests
                        </button>
                        <button onClick={() => setActiveTab('relief')} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === 'relief' ? 'bg-amber-600 text-white shadow-lg' : 'text-[#475569] hover:text-white'}`}>
                            ?? Relief Fund
                        </button>
                    </div>
                </motion.div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="card p-5">
                            <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                                ...
                                <span className="text-xs text-[#64748b] font-normal ml-2">District: {user?.district || 'All'}</span>
                            </h2>

                            {harvestApps.length === 0 ? (
                                <p className="text-[#64748b] text-center py-8">No pending harvest applications</p>
                            ) : (
                                    ...
                                    {harvestApps.map((app) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-4 rounded-2xl border transition-all cursor-pointer ${selectedApp?._id === app._id ? 'border-green-500 bg-green-500/10' : 'border-[#e2e8f0] bg-[#ffffff] hover:border-slate-200'}`}
                                            >
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <div className="font-bold text-slate-900 capitalize">{app.cropType} {app.cropVariety ? `(${app.cropVariety})` : ''}</div>
                                                        <div className="text-xs text-[#64748b]">
                                                            {app.farmerName} � {app.farmerDistrict} � {app.quantityKg}kg � ?{app.farmerPricePerKg}/kg
                                                        </div>
                                                        <div className="text-[10px] text-[#64748b] font-mono mt-1">{app.applicationId}</div>
                                                    </div>
                        ...
                        {/* Inspection Form */}
                        {selectedApp && (
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card p-6 border-green-500/20">
                                ...
                                <div className="p-4 rounded-2xl bg-blue-500/5 border border-blue-500/20 mb-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-[#64748b] uppercase">Name & Contact</span>
                                            <span className="text-slate-900 text-sm font-bold">{selectedApp.farmerName}</span>
                                            <span className="text-blue-400 text-xs">{selectedApp.farmerMobile}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-[#64748b] uppercase">Location</span>
                                            <span className="text-slate-900 text-sm capitalize">{selectedApp.farmerVillage}, {selectedApp.farmerTaluk}</span>
                                            <span className="text-[#475569] text-xs">{selectedApp.farmerDistrict}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-[#64748b] uppercase">Aadhaar & Uzhavar Card</span>
                                            <span className="text-slate-900 text-sm font-mono">{selectedApp.farmerAadhaar || 'N/A'}</span>
                                            <span className="text-green-400 text-xs font-mono">{selectedApp.farmerUzhavarCard || 'N/A'}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-[#64748b] uppercase">Land & Crop Context</span>
                                            <span className="text-slate-900 text-sm line-clamp-1" title={selectedApp.farmerLandDetails}>L: {selectedApp.farmerLandDetails || 'N/A'}</span>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                                    {[
                                        ...
                                        <div key={i} className="bg-[#ffffff] p-3 rounded-xl">
                                            <div className="text-[10px] text-[#64748b]">{item.icon} {item.label}</div>
                                            <div className="text-slate-900 text-sm font-bold capitalize">{item.value}</div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-4 mb-6">
                                    <div>
                                        <label className="block text-[10px] text-[#64748b] mb-1 uppercase font-bold">Quality Grade</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-[#64748b] mb-1 uppercase font-bold">pH Level</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-[#64748b] mb-1 uppercase font-bold">Pesticide Result</label>
                                    ...
                                    <div className="flex items-center">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            ...
                                            <span className="text-sm text-slate-900">?? Organic Verified</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 mb-6">
                                    ...
                                    <div className="flex items-center gap-4 mb-2">
                                        <div className="text-sm text-[#475569]">
                                            Farmer Price: <span className="text-slate-900 font-bold">?{selectedApp.farmerPricePerKg}/kg</span>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <span className="text-[#475569] text-sm">per kg</span>
                                    </div>
                                    <p className="text-[10px] text-[#64748b] mt-2">
                                        ?? Maximum adjustment: �0.2% of farmer's price. This protects farmer income.
                                    </p>
                                ...
                                {/* Inspection Notes */}
                                <div className="mb-4">
                                    <label className="block text-[10px] text-[#64748b] mb-1.5 uppercase font-bold">Inspection Notes *</label>
                                    <textarea
                                ...
                                <div className="flex gap-4 mb-4">
                                    <button
                                        ...
                                        className={`flex-1 py-3 rounded-xl border font-bold transition-all flex items-center justify-center gap-2 ${cropInspection.approved ? 'border-green-500 bg-green-500/10 text-green-400' : 'border-[#e2e8f0] text-[#64748b]'}`}
                                    >
                                        <Award className="w-4 h-4" /> Certify & Approve
                                    </button>
                                    <button
                                        onClick={() => setCropInspection(ci => ({ ...ci, approved: false }))}
                                        className={`flex-1 py-3 rounded-xl border font-bold transition-all flex items-center justify-center gap-2 ${!cropInspection.approved ? 'border-red-500 bg-red-500/10 text-red-500' : 'border-[#e2e8f0] text-[#64748b]'}`}
                                    >
                ...
                {activeTab === 'quality' && (
                    <>
                        <div className="card p-5 mb-6">
                            <h2 className="font-semibold text-slate-900 mb-3">?? Incoming Batches</h2>
                            <div className="space-y-2">
                                {liveBatches.map((b: any) => (
                                    <button
                                        key={b.batchId}
                                        onClick={() => setSelectedBatch(b.batchId)}
                                        className={`w-full flex items-center justify-between p-3 rounded-xl border text-sm transition-all ${selectedBatch === b.batchId ? "border-blue-500/60 bg-blue-900/10" : "border-slate-200 bg-[#ffffff] hover:border-slate-300"}`}
                                    >
                                        <div className="text-left">
                                            <div className="font-medium text-slate-900">{b.batchId} � {b.cropType}</div>
                                            <div className="text-[#64748b] text-xs">{b.farmerName || 'Farmer'}, {b.district} � {b.weightKg} kg</div>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        {!submitted ? (
                            <div className="card p-6">
                                <h2 className="font-semibold text-slate-900 mb-4 flex items-center gap-2"><FlaskConical className="w-5 h-5 text-blue-400" /> Quality Test � {selectedBatch}</h2>
                                <div className="grid md:grid-cols-2 gap-4 mb-6">
                                    {[
                                        ...
                                        <div key={f.key}><label className="block text-sm text-[#475569] mb-1.5">{f.label}</label><input value={(tests as any)[f.key]} onChange={e => setTests(t => ({ ...t, [f.key]: e.target.value }))} placeholder={f.placeholder} type={f.type} className="agri-input" /></div>
                                    ))}
                                    <div><label className="block text-sm text-[#475569] mb-1.5">Grade</label><select value={tests.grade} onChange={e => setTests(t => ({ ...t, grade: e.target.value }))} className="agri-select">{["A+", "A", "B+", "B", "C"].map(g => <option key={g}>{g}</option>)}</select></div>
                                </div>
                                <button onClick={handleSubmit} disabled={loading} className="w-full btn btn-accent text-white font-semibold py-4 rounded-xl flex items-center justify-center gap-2">
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card p-8 text-center">
                                ...
                                <h2 className="text-2xl font-bold text-slate-900 mb-2">Quality Test Recorded!</h2>
                                <p className="text-[#475569] mb-6">pH: {tests.ph} � Pesticide: {tests.pesticide} � Grade: {tests.grade}</p>
                                <button onClick={() => setSubmitted(false)} className="border border-slate-200 text-[#475569] px-6 py-3 rounded-xl hover:bg-[#ffffff]">Next Batch</button>
                            </motion.div>
                        )}
                    </>
                )}
                ...
                {activeTab === 'relief' && (
                    <div className="space-y-6">
                        <div className="card p-5">
                            <h2 className="text-xl font-bold text-slate-900 mb-4">?????????? ????????????? (Relief Claims)</h2>
                            <div className="space-y-3">
                                {claims.length === 0 ? (
                                    <p className="text-[#64748b] text-center py-8">No relief claims</p>
                                ) : (
                                    claims.map((c) => (
                                        <div key={c._id} className={`p-4 rounded-2xl border transition-all cursor-pointer ${selectedClaim?._id === c._id ? 'border-amber-500 bg-amber-500/10' : 'border-[#e2e8f0] bg-[#ffffff] hover:border-slate-200'}`} onClick={() => setSelectedClaim(c)}>
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="font-bold text-slate-900">{c.farmerName} � {c.cropType}</div>
                                                    <div className="text-xs text-[#64748b]">{c.village}, {c.district} � {c.claimRefNo}</div>
                                                </div>
                                                <span className={`text-[10px] px-2 py-1 rounded-full border ${c.status === 'pending' ? 'border-yellow-500/30 text-yellow-400' : c.status.includes('approved') ? 'border-green-500/30 text-green-400' : 'border-slate-200 text-[#64748b]'}`}>
                                                    {c.status}
                                                </span>
                                            </div>
                                        </div>
                        ...
                        {selectedClaim && (
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card p-6 border-amber-500/20">
                                <h3 className="text-lg font-bold text-slate-900 mb-4">Inspection: {selectedClaim.claimRefNo}</h3>
                                <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
                                    <div className="bg-[#ffffff] p-3 rounded-xl"><div className="text-[#64748b]">Damage</div><div className="text-slate-900 font-medium capitalize">{selectedClaim.damageReason?.replace('_', ' ')}</div></div>
                                    <div className="bg-[#ffffff] p-3 rounded-xl"><div className="text-[#64748b]">Date</div><div className="text-slate-900 font-medium">{selectedClaim.damageDate ? new Date(selectedClaim.damageDate).toLocaleDateString() : 'N/A'}</div></div>
                                </div>
                                <div className="space-y-4">
                                    <div><label className="block text-sm text-[#475569] mb-1.5 font-bold">INSPECTION NOTES</label><textarea value={inspection.notes} onChange={e => setInspection(i => ({ ...i, notes: e.target.value }))} className="agri-input h-24" placeholder="Field inspection notes..." /></div>
                                    <div><label className="block text-sm text-[#475569] mb-1.5 font-bold">ESTIMATED AMOUNT ?</label><input type="number" value={inspection.amount} onChange={e => setInspection(i => ({ ...i, amount: e.target.value }))} className="agri-input" placeholder="e.g. 25000" /></div>
                                    <div className="flex gap-4">
                                        <button onClick={() => setInspection(i => ({ ...i, approved: true }))} className={`flex-1 py-3 rounded-xl border font-bold transition-all ${inspection.approved ? 'border-green-500 bg-green-500/10 text-green-400' : 'border-[#e2e8f0] text-[#64748b]'}`}>? Approve</button>
                                        <button onClick={() => setInspection(i => ({ ...i, approved: false }))} className={`flex-1 py-3 rounded-xl border font-bold transition-all ${!inspection.approved ? 'border-red-500 bg-red-500/10 text-red-500' : 'border-[#e2e8f0] text-[#64748b]'}`}>? Reject</button>
                                    </div>
