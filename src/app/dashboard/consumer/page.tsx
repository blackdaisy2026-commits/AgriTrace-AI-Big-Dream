    ...
    return (
        <div className="w-full">
            <div className="flex justify-between text-[10px] mb-1">
                <span className="text-[#64748b]">Stock remaining</span>
            ...
            <div className="text-[9px] text-[#64748b] mt-0.5">Total: {total}kg � Sold: {sold}kg</div>
        </div>
    );
}
...
function QtySelector({ available, value, onChange }: { available: number; value: number; onChange: (v: number) => void }) {
    ...
    return (
        <div className="flex items-center gap-2">
            <button onClick={() => onChange(Math.max(1, value - step))} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-white/20 text-slate-900 flex items-center justify-center"><Minus className="w-3.5 h-3.5" /></button>
            <input type="number" value={value} min={1} max={available}
                ...
                className="w-20 text-center bg-black/30 border border-slate-200 rounded-lg text-slate-900 text-sm py-1.5 focus:outline-none focus:border-green-500/50" />
            <button onClick={() => onChange(Math.min(available, value + step))} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-white/20 text-slate-900 flex items-center justify-center"><Plus className="w-3.5 h-3.5" /></button>
            <span className="text-xs text-[#64748b]">/ {available} kg</span>
        </div>
    );
}
            ...
            <div className="min-h-screen min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="card p-8 text-center">
                        <h2 className="text-xl font-bold text-slate-900 mb-2">Consumer Access Required</h2>
                        <Link href="/login?role=consumer"><button className="btn btn-primary text-white mt-4 px-6 py-2 rounded-xl">Login as Consumer</button></Link>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="min-h-screen min-h-screen">
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 text-center">
                    ...
                    <h1 className="text-3xl font-bold text-slate-900 mb-1">Know & Buy Your Food</h1>
                    <p className="text-[#475569]">Verified crops from Tamil Nadu farmers � Fixed fair prices</p>
                </motion.div>
                ...
                <div className="card p-6 mb-6">
                    ...
                    <Link href="/scan">
                        <motion.button whileTap={{ scale: 0.97 }} className="w-full py-6 rounded-2xl border-2 border-dashed border-amber-500/40 text-center hover:border-amber-500/80 transition-all mb-4 group">
                            ...
                            <div className="text-slate-900 font-medium">Open Camera Scanner</div>
                            <div className="text-xs text-[#64748b] mt-1">Point at product QR code</div>
                        </motion.button>
                    </Link>
                    <div className="flex gap-2">
                        ...
                        <Link href={`/trace/${scanInput || "TN-DEMO001"}`}>
                            <button className="px-4 py-2.5 btn btn-primary text-white rounded-xl text-sm">Go ?</button>
                        </Link>
                    </div>
                </div>
                ...
                {/* Purchase Success */}
                {purchaseSuccess && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card p-6 mb-6 border-green-500/20 bg-green-500/5">
                        <div className="flex items-center gap-3 mb-4">
                            <CheckCircle className="w-8 h-8 text-green-400" />
                            <div>
                                <h3 className="font-bold text-green-400 text-lg">Purchase Confirmed!</h3>
                                <p className="text-xs text-[#475569] font-mono">{purchaseSuccess.purchaseRef}</p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                            <div className="bg-[#ffffff] p-3 rounded-xl"><span className="text-[#64748b]">Crop:</span> <span className="text-slate-900 capitalize">{purchaseSuccess.cropType}</span></div>
                            <div className="bg-[#ffffff] p-3 rounded-xl"><span className="text-[#64748b]">Qty:</span> <span className="text-slate-900">{purchaseSuccess.purchaseQuantityKg}kg</span></div>
                            <div className="bg-[#ffffff] p-3 rounded-xl"><span className="text-[#64748b]">Price:</span> <span className="text-slate-900">?{purchaseSuccess.finalPricePerKg}/kg</span></div>
                            <div className="bg-[#ffffff] p-3 rounded-xl"><span className="text-[#64748b]">Total:</span> <span className="text-green-400 font-bold">?{purchaseSuccess.purchaseTotalAmount?.toLocaleString('en-IN')}</span></div>
                        </div>
                        <button onClick={() => setPurchaseSuccess(null)} className="text-xs text-[#64748b] hover:text-slate-900 transition-colors">Dismiss �</button>
                    </motion.div>
                )}
                ...
                <div className="card p-6 mb-6">
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            <Sprout className="w-16 h-16 text-slate-700 mx-auto mb-4" />
                            <h3 className="text-lg font-bold text-[#64748b]">Marketplace empty</h3>
                            <p className="text-sm text-[#64748b]">New certified crops will appear here</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {marketplace.map((item) => (
                                <motion.div
                                    ...
                                    className="p-5 rounded-2xl bg-[#ffffff] border border-slate-200 hover:border-green-500/20 transition-all"
                                >
                                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-2">
                                                <h3 className="font-bold text-slate-900 text-lg capitalize">{item.cropType}</h3>
                                            ...
                                            <div className="text-sm text-[#475569] mb-2">
                                                From <span className="text-slate-900">{item.farmerName}</span> � {item.farmerDistrict}
                                            </div>
                                            <div className="flex flex-wrap gap-x-6 gap-y-3 mb-4 p-3 rounded-xl bg-green-500/5 border border-green-500/10">
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] text-[#64748b] uppercase">Farmer & Contact</span>
                                                    <span className="text-slate-900 text-sm font-bold">{item.farmerName}</span>
                                                    <span className="text-green-400 text-xs">{item.farmerMobile}</span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] text-[#64748b] uppercase">Origin</span>
                                                    <span className="text-slate-900 text-sm">{item.farmerVillage}</span>
                                                    <span className="text-[#475569] text-xs">{item.farmerTaluk}, {item.farmerDistrict}</span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] text-[#64748b] uppercase">Verification</span>
                                                    <div className="text-slate-900 text-xs font-mono">ID: {item.farmerUzhavarCard || 'N/A'}</div>
                                                    <div className="text-blue-400 text-xs font-mono line-clamp-1">{item.certificationNo}</div>
                                                </div>
                                            </div>

                                            <div className="text-[10px] text-[#64748b] mb-2 px-1">
                                            ...
                                            <div className="flex flex-wrap gap-2 text-[10px]">
                                                <span className="bg-[#ffffff] px-2 py-1 rounded-lg text-slate-800 font-bold border border-[#e2e8f0]">
                                                    ?? {getAvailable(item)}kg available
                                                </span>
                                                <span className="bg-[#ffffff] px-2 py-1 rounded-lg text-green-400 font-bold border border-green-500/20">? Grade {item.qualityGrade}</span>
                                        ...
                                        <div className="flex sm:flex-col items-start sm:items-end justify-between gap-3 min-w-[160px]">
                                            <div className="text-right">
                                                <div className="text-[10px] text-[#64748b] uppercase font-bold">Fixed Price</div>
                                                <div className="text-2xl font-black text-green-400">?{item.finalPricePerKg}<span className="text-xs font-normal text-[#475569]">/kg</span></div>
                                            ...
                                            <div>
                                                <div className="text-[10px] text-[#64748b] mb-1">Qty (kg)</div>
                                                <QtySelector
                ...
                <div className="card p-5">
                    <h2 className="font-semibold text-slate-900 mb-3">What You Can Verify</h2>
