...
export default function RegulatorDashboard() {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            default: return 'text-[#475569] bg-gray-400/10 border-gray-400/20';
        }
    };
            ...
            <div className="min-h-screen min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="card p-8 text-center">
                        <h2 className="text-xl font-bold text-slate-900 mb-2">IAgS Access Required</h2>
                        <Link href="/login?role=regulator"><button className="btn btn-primary text-white mt-4 px-6 py-2 rounded-xl">Login as IAgS / Admin</button></Link>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="min-h-screen min-h-screen">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 mb-1" style={{ fontFamily: "Outfit, sans-serif" }}>??? IAgS Admin Panel (TNAS)</h1>
                        <p className="text-[#475569]">Welcome, <span className="text-red-400">{user?.name}</span> � Monitoring Agriculture Supply Chain ??</p>
                    </div>
                    <div className="flex bg-[#ffffff] p-1 rounded-xl border border-slate-200">
                        <button
                            onClick={() => setActiveTab('monitor')}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'monitor' ? 'bg-red-600 text-slate-900 shadow-lg' : 'text-[#475569] hover:text-slate-900'}`}
                        >
                            <Activity className="w-4 h-4" /> Chain Monitor
                        </button>
                        <button
                            onClick={() => setActiveTab('relief')}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'relief' ? 'bg-green-600 text-white shadow-lg' : 'text-[#475569] hover:text-white'}`}
                        >
                ...
                {activeTab === 'monitor' ? (
                    <>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                            {[
                                { label: "Total Apps", value: stats?.total || 0, icon: <Package />, col: "text-slate-900" },
                                ...
                                <div key={i} className="card p-4 text-center border-[#e2e8f0] bg-[#ffffff]">
                                    ...
                                    <div className="text-[10px] text-[#64748b] uppercase tracking-tighter mt-1">{s.label}</div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 mb-6">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="agri-input !pl-24 w-full py-4 bg-[#ffffff]"
                                />
                            </div>
                        </div>
                        ...
                        <div className="card overflow-hidden border-slate-200">
                            <div className="p-4 border-b border-slate-200 bg-[#ffffff] flex items-center justify-between">
                                ...
                                <div className="text-[10px] text-[#64748b]">Live Blockchain Monitoring � Tamil Nadu Agriculture</div>
                            </div>

                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-[#ffffff] text-[10px] text-[#475569] uppercase font-bold">
                                            <th className="p-4 border-b border-slate-200">Ref ID / Batch</th>
                                            <th className="p-4 border-b border-slate-200">Farmer & Location</th>
                                            <th className="p-4 border-b border-slate-200">Crop & Qty</th>
                                            <th className="p-4 border-b border-slate-200">Status</th>
                                            <th className="p-4 border-b border-slate-200 text-center">Events</th>
                                            <th className="p-4 border-b border-slate-200 text-right">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-white/5">
                                        {filteredApps.map((app) => (
                                            <tr key={app._id} className="hover:bg-[#ffffff] transition-colors group">
                                                <td className="p-4 min-w-[150px]">
                                                    <div className="text-slate-900 font-mono text-xs font-bold">{app.applicationId}</div>
                                                    {app.batchId && (
                                                ...
                                                <td className="p-4">
                                                    ...
                                                    <div className="text-[10px] text-[#475569] flex items-center gap-1 mt-1">
                                                ...
                                                <td className="p-4">
                                                    <div className="text-sm text-slate-800 capitalize font-bold">{app.cropType}</div>
                                                ...
                                                <td className="p-4 text-center">
                                                    <div className="flex flex-col items-center">
                                                        ...
                                                        <div className="text-[8px] text-[#64748b] uppercase">Steps</div>
                                                    </div>
                                                </td>
                                        ...
                                        {filteredApps.length === 0 && (
                                            <tr>
                                                <td colSpan={6} className="p-12 text-center text-[#64748b] italic">No applications found in monitor</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                    ...
                    <div className="grid lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-1 space-y-4">
                            <div className="card p-5">
                                ...
                                <div className="space-y-3">
                                    {claims.length === 0 ? (
                                        <p className="text-[#64748b] text-center py-8">????????????? ??????? ?????</p>
                                    ) : (
                                        claims.map((c) => (
                                            <div
                                                key={c._id}
                                                className={`p-4 rounded-xl border transition-all cursor-pointer ${selectedClaim?._id === c._id ? 'border-green-500 bg-green-500/10' : 'border-[#e2e8f0] bg-[#ffffff] hover:border-slate-200'}`}
                                                onClick={() => {
                                                    setSelectedClaim(c);
                                                    setDecision(d => ({ ...d, amount: (c.tahsildarApprovedAmount || c.estimatedAmount || "").toString() }));
                                                }}
                                            >
                                                <div className="font-bold text-slate-900 text-sm">{c.farmerName}</div>
                                                <div className="text-[10px] text-[#64748b] mb-2">{c.claimRefNo} � {c.district}</div>
                                                <div className="flex items-center justify-between">
                                                    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${c.status === 'tahsildar_approved' ? 'border-blue-500/30 text-blue-400' :
                                                        c.status === 'disbursed' ? 'border-green-500/30 text-green-400' : 'border-slate-200 text-[#64748b]'
                                                        }`}>
                                                        {c.status.replace('_', ' ')}
                                                    </span>
                        ...
                        <div className="lg:col-span-2">
                            {selectedClaim ? (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="card p-6 border-green-500/20 h-full">
                                    <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center justify-between">
                                        ...
                                        <span className="text-xs bg-[#ffffff] px-3 py-1 rounded-full font-normal">Farmer ID: {selectedClaim.uzhavarAttaiNo}</span>
                                    </h3>

                                    <div className="grid md:grid-cols-2 gap-4 mb-6">
                                        <div className="bg-[#ffffff] p-4 rounded-2xl border border-[#e2e8f0]">
                                            <div className="text-xs text-[#64748b] mb-1 uppercase font-bold tracking-wider">Farmer Info</div>
                                            <div className="text-slate-900 font-medium">{selectedClaim.farmerName}</div>
                                            <div className="text-xs text-[#475569]">{selectedClaim.mobileNo} � Aadhaar: {selectedClaim.aadhaarNo}</div>
                                        </div>
                                        <div className="bg-[#ffffff] p-4 rounded-2xl border border-[#e2e8f0]">
                                            <div className="text-xs text-[#64748b] mb-1 uppercase font-bold tracking-wider">Land & Crop</div>
                                            <div className="text-slate-900 font-medium">{selectedClaim.cropType} � {selectedClaim.landAreaAcres} Acres</div>
                                            <div className="text-xs text-[#475569]">Patta: {selectedClaim.pattaNo} � Survey: {selectedClaim.surveyNo}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mb-6 p-4 rounded-2xl bg-blue-900/10 border border-blue-500/20">
                                        <div className="text-xs text-blue-400 mb-2 uppercase font-bold tracking-wider flex items-center gap-2">
                                            ??? Agri Officer Inspection Report
                                        </div>
                                        <div className="grid grid-cols-2 gap-4 mb-3">
                                            <div>
                                                <div className="text-[10px] text-[#64748b]">Inspector</div>
                                                <div className="text-slate-900 text-sm font-medium">{selectedClaim.processorName || 'Pending Review'}</div>
                                            </div>
                                            <div>
                                                <div className="text-[10px] text-[#64748b]">Est. Loss Amount</div>
                                        ...
                                        <div className="text-[10px] text-[#64748b] mb-1">Observation Notes</div>
                                        <div className="text-slate-900 text-xs italic">"{selectedClaim.inspectionNotes || 'No notes provided by officer'}"</div>
                                    </div>
                                    ...
                                    {selectedClaim.status === 'disbursed' ? (
                                        <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-2xl text-center">
                                            ...
                                            <div className="text-slate-900 text-lg font-black mt-2">?{selectedClaim.approvedAmount}</div>
                                            <div className="text-xs text-[#64748b] mt-1 font-mono">{selectedClaim.transactionRef}</div>
                                        </div>
                                    ) : (
                                        <div className="space-y-4">
                                            <div className="text-[#475569] text-sm mb-4">
                                            ...
                                            <div className="grid md:grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-[10px] text-[#64748b] mb-1.5 uppercase font-bold">APPROVED AMOUNT (???????????? ????) ?</label>
                                                    <input
                                                        type="number"
                                                ...
                                                <div>
                                                    <label className="block text-[10px] text-[#64748b] mb-1.5 uppercase font-bold">TRANSACTION REF / CHEQUE NO</label>
                                                    <input
                                                        type="text"
                                            ...
                                            <div>
                                                <label className="block text-[10px] text-[#64748b] mb-1.5 uppercase font-bold">ADMIN APPROVAL NOTES</label>
                                                <textarea
                                            ...
                                            <div className="flex gap-4">
                                                <button
                                                    ...
                                                    className={`px-6 py-4 rounded-xl border font-bold ${!decision.approved ? 'bg-red-500/10 border-red-500 text-red-500' : 'border-[#e2e8f0] text-[#64748b]'}`}
                                                >
                                                    Reject
                                                </button>
                                                <button
                                ...
                                <div className="card p-12 text-center h-full flex flex-col items-center justify-center border-dashed border-[#e2e8f0]">
                                    <CreditCard className="w-16 h-16 text-slate-700 mb-4" />
                                    <h3 className="text-xl font-bold text-[#64748b]">Select a claim to review details</h3>
                                    <p className="text-sm text-[#64748b]">Pending applications from disaster zones</p>
                                </div>
                            )}
                        </div>
                    </div>
