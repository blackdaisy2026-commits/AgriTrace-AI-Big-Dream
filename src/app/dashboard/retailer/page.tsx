...
function QtySelector({ available, value, onChange }: { available: number; value: number; onChange: (v: number) => void }) {
    ...
    return (
        <div className="flex items-center gap-2 mt-2">
            <button onClick={dec} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-white/20 text-slate-900 flex items-center justify-center transition-all"><Minus className="w-3.5 h-3.5" /></button>
            <input
                type="number"
                value={value}
                min={1}
            ...
            />
            <button onClick={inc} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-white/20 text-slate-900 flex items-center justify-center transition-all"><Plus className="w-3.5 h-3.5" /></button>
            <span className="text-xs text-[#64748b]">/ {available} kg max</span>
        </div>
    );
}
            ...
            <div className="min-h-screen min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="card p-8 text-center">
                        <h2 className="text-xl font-bold text-slate-900 mb-2">Retailer Access Required</h2>
                        <Link href="/login?role=retailer"><button className="btn btn-primary text-white mt-4 px-6 py-2 rounded-xl">Login as Retailer</button></Link>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="min-h-screen min-h-screen">
            ...
            <div className="max-w-5xl mx-auto px-4 py-8">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 mb-1" style={{ fontFamily: "Outfit, sans-serif" }}>?? Retailer Dashboard</h1>
                        <p className="text-[#475569]">Welcome, <span className="text-purple-400">{user?.name}</span></p>
                    </div>
                    <div className="flex bg-[#ffffff] p-1 rounded-xl border border-slate-200">
                        <button onClick={() => setActiveTab('marketplace')}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'marketplace' ? 'bg-green-600 text-white shadow-lg' : 'text-[#475569] hover:text-white'}`}>
                            <Sprout className="w-4 h-4" /> Marketplace
                        </button>
                        <button onClick={() => setActiveTab('inventory')}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'inventory' ? 'bg-purple-600 text-slate-900 shadow-lg' : 'text-[#475569] hover:text-slate-900'}`}>
                ...
                {activeTab === 'marketplace' ? (
                    <div className="card p-5 mb-6">
                        ...
                        {loading ? (
                            <div className="text-center py-12"><Loader2 className="w-8 h-8 animate-spin text-green-400 mx-auto mb-2" /><p className="text-[#64748b] text-sm">Loading marketplace...</p></div>
                        ) : marketplace.length === 0 ? (
                            <div className="text-center py-12">
                                <Sprout className="w-16 h-16 text-slate-700 mx-auto mb-4" />
                                <h3 className="text-lg font-bold text-[#64748b]">No crops available</h3>
                                <p className="text-sm text-[#64748b]">Crops appear after farmer records batch.</p>
                            </div>
                        ) : (
                                ...
                                {marketplace.map((item) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            className={`p-5 rounded-2xl border transition-all ${isPartial ? 'bg-amber-500/5 border-amber-500/20' : 'bg-[#ffffff] border-slate-200 hover:border-green-500/30'}`}
                                        >
                                            {/* Header row */}
                                            <div className="flex flex-wrap items-start gap-2 mb-3">
                                                <h3 className="font-black text-slate-900 text-xl capitalize">{item.cropType}</h3>
                                                {item.cropVariety && <span className="text-xs text-[#475569] mt-1">({item.cropVariety})</span>}
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] mb-3">
                                                <div className="bg-[#ffffff] px-2 py-1.5 rounded-lg border border-[#e2e8f0]">
                                                    <span className="text-[#64748b] block text-[9px] uppercase">Farmer</span>
                                                    <span className="text-slate-900 font-bold">{item.farmerName}</span>
                                                    <div className="text-blue-400">{item.farmerMobile}</div>
                                                </div>
                                                <div className="bg-[#ffffff] px-2 py-1.5 rounded-lg border border-[#e2e8f0]">
                                                    <span className="text-[#64748b] block text-[9px] uppercase">Location</span>
                                                    <div className="text-slate-900 truncate">{item.farmerVillage}</div>
                                                    <div className="text-[#475569] truncate">{item.farmerDistrict}</div>
                                                </div>
                                                <div className="bg-[#ffffff] px-2 py-1.5 rounded-lg border border-[#e2e8f0]">
                                                    <span className="text-[#64748b] block text-[9px] uppercase">Quality</span>
                                                    <div className="text-green-400 font-bold">Grade {item.qualityGrade}</div>
                                                    <div className="text-[#475569]">{item.certificationNo}</div>
                                                </div>
                                                <div className="bg-[#ffffff] px-2 py-1.5 rounded-lg border border-[#e2e8f0]">
                                                    <span className="text-[#64748b] block text-[9px] uppercase">Batch</span>
                                                    <div className="text-purple-400 font-mono text-[10px]">{item.batchId}</div>
                                                    <div className="text-[#64748b]">{item.blockchainEvents?.length || 0} chain events</div>
                                                </div>
                                            </div>
                                            ...
                                            {/* Purchase section */}
                                            <div className="flex flex-wrap md:flex-nowrap items-end justify-between gap-4 pt-3 border-t border-slate-200">
                                                <div className="flex-1">
                                                    <div className="text-[10px] text-[#64748b] uppercase font-bold mb-1">Select Quantity to Purchase</div>
                                                    <QtySelector
                                                        available={available}
                                                        value={qty}
                                                    ...
                                                    <div className="mt-2 text-[11px] text-[#475569]">
                                                        ...
                                                        <span className="text-[#64748b] ml-1">({qty}kg � ?{item.finalPricePerKg}/kg)</span>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col items-end gap-2">
                                                    <div className="text-right">
                                                        <div className="text-[10px] text-[#64748b] uppercase">Fixed Price</div>
                                                        <div className="text-3xl font-black text-green-400">?{item.finalPricePerKg}</div>
                                                        <div className="text-xs text-[#64748b]">per kg</div>
                                                    </div>
                                                    <button
                        ...
                        <div className="card p-5 mb-6">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Link href={`/trace/${scanInput || "TN-DEMO001"}`}>
                                    <button className="px-4 py-2.5 btn btn-primary text-white rounded-xl text-sm whitespace-nowrap">Search ?</button>
                                </Link>
                            </div>
                        </div>
                        <div className="card p-5">
                            <h2 className="font-semibold text-slate-900 mb-4">?? Current Inventory</h2>
                            <div className="space-y-3">
                                {liveInventory.length === 0 ? (
                                    <p className="text-[#64748b] text-center py-8">No inventory. Purchase crops from the Marketplace.</p>
                                ) : liveInventory.map((item, i) => (
                                    <motion.div key={item.id || i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                                        className="flex flex-wrap md:flex-nowrap items-center justify-between p-4 rounded-xl bg-[#ffffff] border border-slate-200 gap-3">
                                        <div>
                                            <div className="font-semibold text-slate-900 capitalize">{item.crop}</div>
                                            <div className="text-xs text-[#475569]">{item.id} � {item.origin} � {item.qty}</div>
                                        </div>
